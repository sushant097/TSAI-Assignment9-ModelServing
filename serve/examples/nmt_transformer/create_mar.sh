...
#Check if a model-store directory does not exist
if [ ! -d "model_store" ] 
then
    mkdir model-store
fi
...
then
    ...
    echo
    echo "creating mar file ...."
    torch-model-archiver -f --model-name TransformerEn2De --version 1.0 --serialized-file wmt19.en-de.joined-dict.single_model/model.pt --export-path model-store --handler model_handler_generalized.py --extra-files wmt19.en-de.joined-dict.single_model/dict.en.txt,wmt19.en-de.joined-dict.single_model/dict.de.txt,wmt19.en-de.joined-dict.single_model/bpecodes,fairseq-build/$file_name,setup_config.json -r requirements.txt
    echo "========> mar file creation completed successfully...."
    echo
    ...
    echo
    echo "creating mar file ...."
    torch-model-archiver -f --model-name TransformerDe2En --version 1.0 --serialized-file wmt19.de-en.joined-dict.single_model/model.pt --export-path model-store --handler model_handler_generalized.py --extra-files wmt19.de-en.joined-dict.single_model/dict.de.txt,wmt19.de-en.joined-dict.single_model/dict.en.txt,wmt19.de-en.joined-dict.single_model/bpecodes,fairseq-build/$file_name,setup_config.json -r requirements.txt
    echo "========> mar file creation completed successfully...."
