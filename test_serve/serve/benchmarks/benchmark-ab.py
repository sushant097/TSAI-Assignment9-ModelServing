...
def local_torserve_start():
    ...
    execute(
        f"torchserve --start --model-store {execution_params['tmp_dir']}/model-store "
...
def prepare_local_dependency():
    shutil.rmtree(
        os.path.join(execution_params["tmp_dir"], "model-store/"), ignore_errors=True
    )
    os.makedirs(
        os.path.join(execution_params["tmp_dir"], "model-store/"), exist_ok=True
    )
    shutil.rmtree(
